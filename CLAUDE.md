...
- `TASKS.md` — пошаговый чеклист реализации, 9 этапов, текущий прогресс и критерии
  готовности каждого пункта. **Отмечать выполненное здесь** (`[ ]` / `[~]` / `[x]`)
  и обновлять «Текущий этап» и таблицу прогресса.

Написано на сейчас: этап 0 (каркас) и почти весь этап 1 — конфиг, модели, миграция,
точка входа, авторегистрация семьи, `timeutil`, `repo`, `texts`, `keyboards`,
просмотр (`handlers/views.py`) и мастер `/new` (`handlers/new_entry.py`).
Пункты стоят как `[~]`: код и тесты есть, живых проверок в группе не было.

Ещё не написано: `services/ticker.py`, `digest.py`, `panel.py`, `llm.py`,
`nlp_fallback.py`, `voice.py`, `handlers/capture.py`, `handlers/lists.py`.
...
Фоновые сервисы (тикер и дайджест, этап 2) идут мимо диспетчера, а значит и мимо
`FamilyMiddleware` — им придётся открывать `Session()` из `bot/db/session.py` самим
и брать таймзону из `families.tz` вручную.
...
- **`bot/db/sqlite.py` импортируется ради побочного эффекта** (`# noqa: F401` в `repo.py`):
  ...
  `lower()` умеет только ASCII, без этого поиск по кириллице не работает. Не удалять
