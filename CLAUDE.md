...
сборка Docker-образа, нужна для VPS). Текущий этап — 2п, живая панель дня.

Этап 2п написан: `services/panel.py`, `sending.edit`, колонка `families.panel_day`
(ревизия `d5cde13bae0c`). Тестами покрыт, живого прогона не было.

Ещё не написано: `llm.py`, `voice.py`, `handlers/capture.py`, `handlers/lists.py`.
