...
class Settings(BaseSettings):
    ...
    # Сколько обращений к модели в сутки себе позволяем. У бесплатного тира
    # OpenRouter лимит **на аккаунт** (50 без пополнения, 1000 после разовой
    # покупки кредитов на $10), и именно из-за него отменён режим `all`.
    ...
    # деградирует до `dateparser`. Берём с запасом от 50: упереться в потолок
    # своим счётчиком лучше, чем чужим, — свой умеет сказать об этом вслух.
    # 0 = не ограничивать (платный ключ)
    llm_daily_limit: int = 45
