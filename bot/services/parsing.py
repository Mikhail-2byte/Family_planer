...
# Поля `list` и `assignee` из ранней схемы разбора здесь намеренно отсутствуют.
...
- Явная просьба записать перевешивает всё остальное в сообщении. Если фраза
  начинается со слов «запиши», «добавь», «напомни», «поставь задачу», «заведи»
  — это create, даже если дальше идёт длинный разговор, спор или рассуждение.
  Возьми из него то, что просили записать, и не отбрасывай сообщение целиком
  из-за разговорного тона: сюда попадают расшифровки голосовых, а вслух люди
  договаривают вслух. В title — суть просьбы, а не вся расшифровка.
...
- kind. Различай по вопросу «это надо сделать или это надо помнить»:
  task — просят что-то сделать, у дела бывает срок: «записать к врачу»,
  «оплатить садик», «позвонить Виктории 31.08»;
  note — сообщают факт, который надо помнить, делать по нему нечего и срока у
  него обычно нет: «пароль от вайфая такой-то», «у Пети размер обуви 34»,
  «Ксюша сказала, что дочь родится 3 сентября»;
  event — встреча, приём, поездка, у которой есть время;
  shopping — продукты и покупки.
  Сомневаешься между task и note — выбирай task.
- У note заполняй due_at, только если дата названа прямо. Заметка — это то, что
  надо помнить, а не дело со сроком: выдуманная дата превратит её в просрочку.
