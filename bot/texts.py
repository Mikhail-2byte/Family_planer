...
# Приписка про уборку обязательна. Копия «вне машины» берётся руками через
# `/backup` — на этом держится весь смысл ежедневного бэкапа, который в чат сам
# не уходит. А с этапа 11 утренняя уборка стирает и этот файл вместе со всем
# прочим: без предупреждения человек однажды обнаружит, что единственной
# внешней копии базы нет, и узнает об этом ровно тогда, когда она понадобится
BACKUP_CAPTION = (
    "Копия базы на {day}. Снята через <code>VACUUM INTO</code> — "
    "открывается любым просмотрщиком SQLite.\n\n"
    "<b>Перешлите файл себе:</b> утром я чищу чат, и он исчезнет вместе "
    "со всем остальным."
)
