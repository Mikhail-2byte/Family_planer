...
class Family(Base):
    ...
    # До какого message_id чат уже вычищен утренней уборкой (этап 11).
    # Не «сколько удалено», а именно граница: следующее утро начинает диапазон
    # со следующего id. Значение — `id сводки минус один`, и это не мелочь:
    # при знаке, равном самой сводке, завтрашний диапазон начнётся выше неё, и
    # вчерашняя сводка не удалится никогда.
    # id принадлежат конкретному чату, поэтому при переезде в супергруппу
    # колонка обнуляется вместе с панелями
    swept_upto: Mapped[int | None] = mapped_column(Integer)
...
class ListModel(Base):
    ...
    panel_message_id: Mapped[int | None] = mapped_column(Integer)
    # Когда список закрыли (этап 10). Нужен схлопнутой панели: она печатает
    # «Закрыт 30 авг» вместо тридцати зачёркнутых строк. Печатается через
    # `day_stamp`, а не «вчера/сегодня»: относительная метка меняется на
    # полуночи, и панель начала бы переписываться сама по себе.
    # У списков, закрытых до этой ревизии, останется NULL — текст обязан
    # переживать эту ветку. Восстанавливать дату задним числом по `max(done_at)`
    # пунктов не стали: это гадание, а следа в выгрузке всё равно нет
